...
def get_current_scion_path():
  ...
  #
  print "current scion path: %s" %(current_path)
  #
  scion_path=list()
      ...
      with open(rootstock_active_file_path,"r") as f:
         ...
         for line in f:
            ...
            try:
               ...
               print "active rootstock path: %s" %(entry)
               return active_rootsock_path
            except IndexError:
              continue
            #
   except IOError:
      print "warning file %s not exist\n" %(rootstock_active_file_path) 
   return ""
  ...
  for entry_name in dirs:
    if os.path.isdir(current_path+"/"+entry_name):
      ...
      if not os.path.exists(current_path+"/"+entry_name+"/"+scion_stem_dir+"/"+scion_hidden_dir):
        ...
        print "found scion path: %s" %(current_path+"/"+entry_name)
...
def extract_scion_entry(line): 
   ...
   print "extract_scion_entry: %s %s %s" %(shelf,scion,version)
   ...
   try:
      with open(file_path,"r") as f:
         ...
         for line in f:
            ...
            print "entry[%d]: %s" %(entry_index,entry)
   ...
   try:
      ...
      with open(sources_list_file_path,"r") as f:
         ...
         for line in f:
            ...
            #
            entry_index=entry_index+1
      print "found location of scion %s (version %s) on shelf %s from %s \n" %(scion,version_selected,shelf,location_selected)
      return location_selected, path_selected
   except IOError:
      print "warrning file %s not exist\n" %(sources_list_file_path)
   ...
   try:
      with open(scion_grafted_list_file_path,"r") as f:
         ...
         for line in f:
            ...
            #
            print "dictionary[%s]: %s" %(entry_key,entry)
            entry_index=entry_index+1
   except IOError:
      print "warning file %s not exist\n" %(scion_grafted_list_file_path) 
   ...
   for entry in scions_list:
      ...
      if entry_key in scions_grafted_dictionary:
         ...
         try:
            ...
            print "error : missing parameters in line %s\n" %(line)

         if(compare(version,version_grafted)>0):
             print "information: scions %s version %s from shelf %s will be installed instead current version %s" %(scion,version,shelf,version_grafted)
            ...
            #nothing to do
             print "information: scions %s version %s from shelf %s: current version %s is more recent." %(scion,version,shelf,version_grafted)
         else:
            print "information: scions %s version %s from shelf %s already existing in depots.\n" %(scion,version,shelf)
...
def ungraft_all_scions(rootstock_path):
   ...
   for file in dirs:      
      ...
      print "islink? %s:%s\n" %(rootstock_path+"/"+file, str(os.path.islink(rootstock_path+"/"+file)))
      if os.path.islink(rootstock_path+"/"+file)==True:
         ...
         try:
            ...
            print "error: cannot remove link on: %s\n" %(rootstock_path+"/"+file)
...
print "current active rootstock path: %s" %(current_active_rootstock)
#
current_scion_path = get_current_scion_path()
if(len(current_scion_path)<=0):
  print "error : it's not scion directory: %s not found" %(scion_stem_dir+"/"+scion_hidden_dir)
  sys.exit(1)

scion_sources_list_file_path= current_scion_path+"/"+scion_hidden_dir+"/"+scion_sources_list_file
print "current scion path: %s\n" %(current_scion_path)
